...
describe('Teste Unitário de Criação de Sessão', () => {
  ...
  test('Não deve ser possivel realizar uma nova sessão com um email inválido', async () => {
    await expect(createSessionUseCase.execute('invalid_mail@mail.com', '123456')).rejects.toEqual(new AppError('E-mail or password is invalid!', 400))
  })

  test('Não deve ser possivel realizar uma nova sessão com uma senha inválido', async () => {
    await expect(createSessionUseCase.execute('john@mail.com', 'invalid_password')).rejects.toEqual(new AppError('E-mail or password is invalid!', 400))
  })
})